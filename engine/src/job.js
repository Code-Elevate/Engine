...
class Job {
  ...
  async safe_call(
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout_call = ["timeout", "-s", "9", Math.ceil(timeout / 1000)];
      ...
      const proc = cp.spawn(proc_call[0], proc_call.splice(1), {
        env: {
          ...
          CODE_LANGUAGE: this.runtime.language,
        },
        stdio: "pipe",
        cwd: this.dir,
        uid: this.uid,
      ...
      proc.stderr.on("data", async (data) => {
        if (event_bus !== null) {
          ...
          try {
            ...
            // Could already be dead and just needs to be waited on
            this.logger.debug(`Got error while SIGKILLing process ${proc}:`, e);
          }
        } else {
          stderr += data;
          output += data;
    ...
    while (processes.length > 0) {
      ...
      processes = proc_ids.map((proc_id) => {
        ...
        try {
          ...
          if (state == "Z") {
            ...
            if (!to_wait.includes(proc_id_int)) to_wait.push(proc_id_int);
      ...
      for (const proc of processes) {
        ...
        try {
          ...
          this.logger.debug(`Got error while SIGSTOPping process ${proc}:`, e);
        }
      }
