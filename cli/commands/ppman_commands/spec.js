...
exports.handler = async ({ axios, specfile }) => {
  ...
  const packages_req = await axios.get("/api/packages");
  ...
  for (const rule of rules) {
    ...
    if (!rule.negate) {
      ...
      ensure_packages = ensure_packages.filter((pkg) => !does_match(pkg, rule));
    }
  }
  ...
  for (const op of operations) {
    console.log(
      (op.type == "install" ? chalk.green("Install") : chalk.red("Uninstall")) +
  ...
  for (const op of operations) {
    if (op.type == "install") {
      try {
        const install = await axios.post(`/api/packages`, {
          language: op.package,
          version: op.version,
        });

        if (!install.data.language) throw new Error(install.data.message); // Go to exception handler

        console.log(chalk.bold.green("Installed"), op.package, op.version);
      } catch (e) {
        console.log(
          chalk.bold.red("Failed to install") + ` ${op.package} ${op.version}:`,
          e.message
        );
      }
    } else if (op.type == "uninstall") {
      try {
        const install = await axios.delete(`/api/packages`, {
          data: {
            language: op.package,
            version: op.version,
          },
